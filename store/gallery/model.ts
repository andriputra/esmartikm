...
export const actions: ActionTree<ModelState, RootState> = {
  async LOAD({ commit }, model: number | Attrs): Promise<any> {
    ...
    try {
      ...
      if (typeof model === 'number') {
        commit('LOADING')
        attrs = await this.$api.gallery.get(model)
        commit('LOADED')
  ...
  async SUBMIT({ state, commit }): Promise<any> {
    ...
    try {
      ...
      const fields = ['title', 'event_name', 'type', 'media', 'description']
      ...
      if (typeof data.media === 'string' && data.type !== 'video')
        delete data.media

      let response
      if (modelAttrs.id) {
        response = await this.$api.gallery.putUpload(modelAttrs.id, data)
      } else {
        response = await this.$api.gallery.postUpload(data)
      }
  ...
  async DELETE({ state, commit }): Promise<any> {
    ...
    try {
      await this.$api.gallery.delete(state.attrs.id)
      commit('DELETED')
      return { error: false }
    } catch (error: any) {
      commit('DELETED')
